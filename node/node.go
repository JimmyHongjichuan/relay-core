...
func NewBraftNode(localNodeInfo cluster.NodeInfo) *BraftNode {
	...
	bs := primitives.NewBlockStore(db, ts, btcWatcher, bchWatcher, ethWatcher,
		//xinWatcher, eosWatcher,
		priceTool,signer, localNodeInfo.Id)
	...
	ld := NewLeader(localNodeInfo, bs, ts, signer, btcWatcher, bchWatcher, ethWatcher,
		//xinWatcher, eosWatcher,
		priceTool, pm)
...
func (bn *BraftNode) watchNewTx(ctx context.Context) {
	...
	//	bn.xinWatcher.StartWatch(uint32(xinHeight), uint32(xinIndex), xinTxChan, 5)
	//}
	...
	//	bn.eosWatcher.StartWatch(uint32(eosHeight), uint32(eosIndex), eosTxChan, 5)
	//}
